...
for name in names:
    for epoch in [89]:
        model = None
        model = get_speid()
        model.load_weights("./model/speid/%sModel%s.h5" %(name, epoch))
        Data_dir='/home/ycm/data/%s/'%name
        test=np.load(Data_dir+'%s_test_speid.npz'%name)
        X_en_tes,X_pr_tes,y_tes=test['X_en_tes_speid'],test['X_pr_tes_speid'],test['y_tes_speid']
